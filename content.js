...
const autoBracket = (() => {
  ...
  function handleBracketInsert({ text, cursorPos, selectionEnd, key }) {
    ...
    if (hasSelection) {
      newText = text.slice(0, cursorPos) + key + selectedText + closingChar + text.slice(selectionEnd);
      newCursorPos = cursorPos + selectedText.length + 2;
      highlightStart = cursorPos;
      highlightEnd = cursorPos + selectedText.length + 2;
    } else {
      newText = text.slice(0, cursorPos) + key + closingChar + text.slice(cursorPos);
  ...
  function handleSmartBackspace({ text, cursorPos }) {
    ...
    if (pairs[prevChar] && nextChar === pairs[prevChar]) {
      ...
      const newText = text.slice(0, cursorPos - 1) + text.slice(cursorPos - 1 + charactersToRemove);
...
(function() {
  ...
  group('autoBracket.handleTabSpace', function() {
    ...
    /* DOES NOT WORK USING NESTED BRACKETS
